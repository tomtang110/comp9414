% Assignment name: Assignement3
% Name: Nanyang Tang, Qixing Li
% Student Number: z5103095, z5104356
% Date: 27/05/2018
...
% s((X,Y),(X1,Y1),1000) is to indicate the cost 
% from land(X,Y) to water(X1,Y1) or water(X1,Y1)
% to land(X,Y) or water(X,Y) to water(X1,Y1).
% the cost of this 3 types of moves is 1000,
% which can make sure (land or dropped) will be
% chosen instead of directly swim across the river
...
% this predicate would determine the agent's
% next Action and new intention,
% based on the previous intention, agent's location,
% and agent_stone().

% if there is no reachable stones(Init_pick is empty),
% the agent will stay where he is.
get_action(intents(Init_drop,Init_pick),intents(Init_drop,Init_pick),move(X1,Y1)):-
    agent_at(X1,Y1),
    agent_stones(0),
    is_Empty(Init_pick).
% while there are reachable stones,
% but the pick plan is empty or the next action is not applicable,
% design a new plan for the Init_pick
...
get_action(intents(Init_drop,[[goal(X,Y),[First_action|Next_action]]|Pick_plan]),intents(Init_drop,[[goal(X,Y),Next_action]|Pick_plan]),First_action):-
    agent_stones(0).
% while agent has stone,
% but the drop plan is empty or the next action is not applicable,
% design a new plan for the Init_drop
...
% paixu(_,_) is to take away the first element
% as it is the action is being executing.
paixu([_|T],T).

% add_move is to add actions one by one into one plan
% if x1 \= x and y1 \= y, add action to plans
...
add_move(MA,[(X1,Y1)|T],X,Y,Result5):-
    ...
    add_move(MA,T,X,Y,Result6),
    Result5 = [move(X1,Y1)|Result6].
% when the agent gets to the target location
% add the last action (pick or drop) into the plan
add_move(MA,[(X1,Y1)|[]],X,Y,Result5):-
    X1 = X,
    Y1 = Y,
    Result5=MA.

% design the new plan for Init_pick or Init_drop
% choose the goal with least cost path
% bb(X,Y) is the aim for the solve() function
...
% this predicate updates the new Intention,
% based on the current intentions and the actions that the agent just finished
update_intentions(at(_,_), Intentions2, Intentions2).
% when taking drop action, take away the last action of drop action.
update_intentions(dropped(_,_), intents([_|Init_drop],Init_pick), intents(Init_drop,Init_pick)).
% when taking pick action, take away the last action of pick action.
